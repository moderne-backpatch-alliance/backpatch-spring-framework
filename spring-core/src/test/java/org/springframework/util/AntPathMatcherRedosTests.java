 ...
 * cannot match.
 *
 * <p>Measured on the released spring-core 6.1.21 jar, the pattern below does not
 * complete within four seconds, while spring-core 6.2.19 aborts it in 14 ms.
 *
 * <p>The match runs on a separate thread so that the vulnerable behaviour shows
...
class AntPathMatcherRedosTests {
	...
	void matchAbortsInsteadOfBacktrackingCatastrophically() throws Exception {
		String pattern = "/x/" + "*a".repeat(12) + "*b";
		String path = "/x/" + "a".repeat(40);
	 ...
	 */
	private Throwable matchWithin(String pattern, String path) throws Exception {
